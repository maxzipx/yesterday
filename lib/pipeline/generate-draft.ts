...
type ClusterRow = Pick<Database["public"]["Tables"]["story_clusters"]["Row"], "id" | "label">;
...
export async function generateDraftFromTopClusters(
  ...
  if (candidates.length < REQUIRED_STORY_COUNT) {
    throw new Error("Not enough ranked clusters for this date. Run ingest -> cluster -> rank first.");
  }
  ...
  if (briefError || !briefData?.id) {
    throw new Error(`Failed to create or update draft brief: ${briefError?.message ?? "Unknown error"}`);
  }
